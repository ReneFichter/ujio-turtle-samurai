...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const shortsVideos = [
    ...
    { url: 'https://www.youtube.com/embed/mH0dCWsDA7w', title: 'Short 3' },
    { url: 'https://www.youtube.com/embed/nD54E-gf8OE', title: 'Short 4' },
    { url: 'https://www.youtube.com/embed/uvKpFOzlFZA', title: 'Short 5' },
    { url: 'https://www.youtube.com/embed/ScMzIvxBSi4', title: 'Short 6' },
    { url: 'https://www.youtube.com/embed/yWrdFB0GSjo', title: 'Short 7' },
    { url: 'https://www.youtube.com/embed/ysz5S6PUM-U', title: 'Short 8' },
    { url: 'https://www.youtube.com/embed/aqz-KE-bpKQ', title: 'Short 9' }
  ];
  let shortsIndex = 0;
  function renderShortsCarousel() {
    const len = shortsVideos.length;
    const prev = (shortsIndex - 1 + len) % len;
    const next = (shortsIndex + 1) % len;
    const main = shortsIndex;
    const container = document.getElementById('shortsCustomCarousel');
    container.innerHTML = '';
    // Linke Vorschau
    const prevDiv = document.createElement('div');
    prevDiv.className = 'shorts-card-side';
    prevDiv.innerHTML = `<div class='shorts-card-iframe'><iframe src='${shortsVideos[prev].url}' title='${shortsVideos[prev].title}' style='pointer-events:none;' tabindex='-1' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></div>`;
    prevDiv.addEventListener('click', function(e) {
      shortsIndex = prev;
      renderShortsCarousel();
    });
    container.appendChild(prevDiv);
    // Hauptvideo
    const mainDiv = document.createElement('div');
    mainDiv.className = 'shorts-card-main';
    mainDiv.innerHTML = `<div class='shorts-card-iframe'><iframe src='${shortsVideos[main].url}' title='${shortsVideos[main].title}' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></div>`;
    container.appendChild(mainDiv);
    // Rechte Vorschau
    const nextDiv = document.createElement('div');
    nextDiv.className = 'shorts-card-side';
    nextDiv.innerHTML = `<div class='shorts-card-iframe'><iframe src='${shortsVideos[next].url}' title='${shortsVideos[next].title}' style='pointer-events:none;' tabindex='-1' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></div>`;
  ...
  document.getElementById('shortsPrevBtn').addEventListener('click', function() {
    shortsIndex = (shortsIndex - 1 + shortsVideos.length) % shortsVideos.length;
    renderShortsCarousel();
  });
  document.getElementById('shortsNextBtn').addEventListener('click', function() {
    shortsIndex = (shortsIndex + 1) % shortsVideos.length;
    renderShortsCarousel();
  });
  // Touch/Swipe-Unterstützung für Handy (Pointer + Touch für iOS/Safari)
  let touchStartX = null;
  let touchEndX = null;
  const swipeArea = document.getElementById('shortsCustomCarousel');
  // Pointer Events (Android, moderne Browser)
  ...
  swipeArea.addEventListener('pointerup', function(e) {
    if ((e.pointerType === 'touch' || e.pointerType === 'pen') && touchStartX !== null) {
      touchEndX = e.clientX;
...
// Autoplay-Fallback für index.html und index_en.html
document.addEventListener('DOMContentLoaded', function() {
  var videoIds = [
    'heroVideo',
    'teaserToyshots',
    ...
    if (video) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(function() {
          // Autoplay wurde blockiert – Fallback anzeigen
          if (!video.parentNode.querySelector('.video-fallback-btn')) {
            var fallback = document.createElement('button');
            fallback.textContent = (document.documentElement.lang === 'en') ? 'Play video' : 'Video abspielen';
            fallback.className = 'video-fallback-btn';
            fallback.style.margin = '8px auto';
