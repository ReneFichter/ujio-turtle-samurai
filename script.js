...
const TOY_GALLERY_COPY = {
  ...
  'Sakonji.jpg': {
    de: {
      title: 'Meiste unter Kirschblüten',
      description: 'Zarte Farben und der dichte Blütenrahmen verleihen der Figur eine ruhige, pötische Wirkung.'
    },
    en: {
...
const modalThemeClass = (() => {
 ...
 const isToyPage = currentPage === 'toys.html' || currentPage === 'toys_en.html';
  ...
  const getGalleryItems = (groupId) => {
    ...
    const placeholderItems = galleryPlaceholders[groupId] ? galleryPlaceholders[groupId].map((item) => ({ ...item })) : [];
...
const messageVal = messageInput?.value?.trim() || '';
    ...
    if (status) {
      status.textContent = 'Danke! Deine Nachricht wurde gesendet. (Demo nur lokal)';
...
function initVideoHover() {
  const videos = document.querySelectorAll('.video-wrapper video:not([controls])');
  ...
  videos.forEach((video) => {
    ...
    wrapper.addEventListener('focusout', resetVideo);
